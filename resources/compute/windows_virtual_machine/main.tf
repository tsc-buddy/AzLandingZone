...
resource "azurerm_windows_virtual_machine" "base" {
  ...
  computer_name                = "${substr(var.name_prefix, 4, 8)}-${format("%02d", count.index)}-${substr(var.environment, 0, 2)}${substr(var.location_short, 0, 2)}"
  ...
  os_disk {
    ...
    name                      = "osdisk-${var.name_prefix}-${format("%02d", count.index)}-${var.environment}-${var.location_short}"
